...
class PHPWebSocket
{
    ...
    {
        include('config.php');
        $sc = $sugar_config['dbconfig'];
        $con = mysqli_connect("localhost", $sc['db_user_name'], $sc['db_password'], $sc['db_name']);
        ...
        $qry = 'select status, save_id, unique_id, links, actual from asterisk_calls where operator = "' . $id . '" order by date desc limit 1';
        ...
        if ($row['actual'] == 1) {
            ...
            $links = $row['links'];
            $links = rawurlencode($links);
            if (substr($links, 0, 1) == "?") {
                $links = substr($links, 1 - strlen($links));
            }
            $argh = $links . '^|^' . $row['status'] . '^|^' . $saveid . '^|^' . $row['unique_id'];
    ...
    {
        $this->log($this->wsClientCount . ' clients online');

        foreach ($this->wsClients as $clientID => $client) {
            if (isset($this->clientsEx[$clientID])) {
                $row = $this->select($this->clientsEx[$clientID]);
            }
            if (strlen($row) > 300) {
                $this->wsSend($clientID, $row);
                $this->log('Ответ отправлен оператору ' . $this->clientsEx[$clientID]);
            }
        }

        foreach ($this->clientsEx as $clientID) {
            include('config.php');
            $sc = $sugar_config['dbconfig'];
            $con = mysqli_connect("localhost", $sc['db_user_name'], $sc['db_password'], $sc['db_name']);
            $query = 'update asterisk_calls set actual = 0 where operator = "' . $clientID . '"';
    ...
    {
        $this->wsClients[$clientID][12] = $data;
    ...
    {
        echo DateUpdate('Y-m-d H:i:s: ') . $message . "\n";
    }
