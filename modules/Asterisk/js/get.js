...
$(document).ready(function()
{
	...
	var connected = false;
		...
		///////////////////////////////////////////////////////////////////////////
			Server = new FancyWebSocket('ws://' + location.hostname + ':12345');
			...
			Server.bind('close', function( data ) {
				...
				console.log("Отключены от ws://" + location.hostname + ":12345, переподключаемся через 3 секунды");
			});
	...
	window.successGetForm = function successGetForm(response, status, saved_id, uid)
	{
		try
		{
			//если это новый звонок то удаляем элемент "first_call"
			//if (status == 0) { 
			//	$('#first_call').remove();
			//};
			//если элемент "first_call" существует, значит звонков еще не было и форму не рисуем
			//if ($('#first_call').length > 0) {} else {
			//{
				var kkk = uid.split('.');
				var key = kkk[0];
				...
				if ((!$('#' + form_id).length > 0) && ($('#last_uid').val() != uid)) // Создаем и отображаем форму, если ранее этого сделано не было.
				{
					...
					//if($('#last_uid').val() != uid) {//если уид новый, показываем форму
					...
					//if ($('#last_uid').val() == uid){} else {
					$('#replaceme' + form_id).html("");
					$('#replaceme' + form_id).html(response);
					$('#' + form_id).attr('status', status);
					$('#ui-dialog-title-' + form_id).html('Asterisk. ' + getStatus(status));
					
					//alert(saved_id + ' - ' + saved_id.length);
