...
global $sugar_config, $db, $beanList, $timedate, $current_user;
...
if (!empty($_POST)) { // Если POST пришел, то мы можем предположить, что звонок состоялся.
    $linked_id = $_POST['unique_id'];
   
    if ($startDate = getEventDate('BRIDGE_START', $linked_id, $db)) {
        $date_start = $startDate;
    }
    
    if ($endDate = getEventDate('BRIDGE_END', $linked_id, $db)) {
        $date_end = $endDate;
    }
...
// Сохраняем звонок:
$call = new Call();
// Проверка на существование сохраненного
$res = $db->query("SELECT save_id FROM asterisk_calls WHERE linked_id = '$linked_id' ORDER BY date DESC LIMIT 1");

if ($row = $db->fetchByAssoc($res)) {
    $call->retrieve($row['save_id']);
} else {
    $res = $db->query("SELECT id FROM calls WHERE linked_id = '$linked_id' LIMIT 1");
    if ($row = $db->fetchByAssoc($res)) {
...
$call->phone = $number;
$call->date_entered = $timedate->handle_offset($date_start, $timedate->get_db_date_time_format(), true, $current_user);
$call->date_modified = $timedate->handle_offset($date_start, $timedate->get_db_date_time_format(), true, $current_user);
$call->modified_user_id = $_POST['user_id'];
$call->created_by = $_POST['user_id'];
$call->assigned_user_id = $_POST['user_id'];
$call->duration_hours = $duration_hours;
$call->duration_minutes = $duration_minutes;
$call->duration_seconds = $duration_seconds;
$call->date_start = $timedate->handle_offset($date_start, $timedate->get_db_date_time_format(), true, $current_user);
$call->date_end = $timedate->handle_offset($date_start, $timedate->get_db_date_time_format(), true, $current_user);
$call->status = $status;
...
if ($call_id = $call->save()) {
    ...
    $query1 = "UPDATE asterisk_calls SET save_id = '".$call_id."', actual = 1 WHERE linked_id = '".$_POST['unique_id']."'";
 ...
 * @return array
 */
function getEventDate($event, $linked_id, $db) // Функция получения даты начала/конца разговора.
{
